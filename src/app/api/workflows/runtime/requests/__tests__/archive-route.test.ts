...
jest.mock('@/lib/workflows/runtime/use-cases/archive-request', () => ({
...
const { POST } = require('@/app/api/workflows/runtime/requests/[id]/archive/route');
...
describe('archive route', () => {
  ...
  it('returns 200 when request archives successfully', async () => {
    archiveRequest.mockResolvedValue({ requestId: 123, status: 'archived' });

    const response = await POST(
      new Request('http://localhost/api/workflows/runtime/requests/123/archive', {
        method: 'POST',
    ...
    await expect(response.json()).resolves.toMatchObject({
      ok: true,
      data: { requestId: 123, status: 'archived' },
    });
  });
