    ...
    const { control, register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormValues>({
        ...
        defaultValues: {
            data: opportunityData && opportunityData[section] 
                ? Object.entries(opportunityData[section]).map(([key, value]) => ({ key, value }))
                : [],
        },
    });
                                ...
                                <div key={field.id} className="flex items-end gap-2 p-3 border rounded-lg bg-muted/50">
                                    <div className="flex-grow space-y-1.5">
                                        ...
                                        {errors.data?.[index]?.key && (
                                            <p className="text-sm text-destructive">{errors.data[index]?.key?.message}</p>
                                        )}
                                    </div>
                                    <div className="flex-grow space-y-1.5">
                                        ...
                                        {errors.data?.[index]?.value && (
                                            <p className="text-sm text-destructive">{errors.data[index]?.value?.message}</p>
                                        )}
                                    </div>
