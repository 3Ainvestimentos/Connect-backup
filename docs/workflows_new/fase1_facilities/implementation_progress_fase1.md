...
Foi implementada a Etapa 7 da Fase 1 de Facilities, incluindo `facilities_solicitacao_compras` na mesma superficie multiworkflow de `/pilot/facilities`, adicionando a aba `Concluidas` sobre o backend ja existente e consolidando a UX final do piloto para encerrar a Fase 1.

### O que foi implementado

- registry local expandido para os 3 workflows piloto em:
  - `src/lib/workflows/pilot/workflow-registry.ts`
- remocao da duplicacao estrutural do resolver de workflow ativo, passando a derivar o conjunto suportado a partir do registry
- nova leitura client-safe de concluidos em:
  - `src/lib/workflows/pilot/api-client.ts`
- nova query key de concluidos em:
  - `src/lib/workflows/pilot/query-keys.ts`
- novo consumo de concluidos no hook do piloto em:
  - `src/hooks/use-facilities-pilot.ts`
- nova aba operacional de concluidos em:
  - `src/components/pilot/facilities/CompletedTab.tsx`
- pagina do piloto consolidada para:
  - 3 workflows
  - 4 cards de topo
  - aba `Concluidas`
  - mesma filtragem local `all | active`
  em:
  - `src/components/pilot/facilities/FacilitiesPilotPage.tsx`
- formulario dinamico ajustado para o workflow 3:
  - `anexos` opcional
  - upload so quando houver `File`
  - omissao do campo quando vazio
  em:
  - `src/components/pilot/facilities/OpenWorkflowCard.tsx`
- dialog operacional refinado para:
  - exibir o responsavel atual em leitura
  - esconder o seletor apos a primeira atribuicao
  em:
  - `src/components/pilot/facilities/RequestDetailsDialog.tsx`

### Decisoes tecnicas materializadas na Etapa 7

- a mesma rota `/pilot/facilities` passou a operar os 3 workflows:
  - `facilities_manutencao_solicitacoes_gerais`
  - `facilities_solicitacao_suprimentos`
  - `facilities_solicitacao_compras`
- a etapa reaproveitou o endpoint ja existente:
  - `GET /api/workflows/read/completed`
  sem criar backend novo
- o agrupamento de concluidos passou a usar:
  - `closedMonthKey`
  na mesma semantica do backend de leitura
- o workflow 3 reutilizou exatamente os primitives ja validados:
  - `text`
  - `textarea`
  - `select`
  - `file`
- o campo `anexos` de compras foi tratado como:
  - `file` opcional
  - upload sob demanda
  - sem placeholder vazio no `formData`
- a UX de atribuicao foi consolidada para:
  - seletor apenas enquanto nao houver responsavel
  - leitura do responsavel apos atribuicao
...
Foi executado o roteiro completo de smoke da Fase 1, cobrindo os 3 workflows e a superficie consolidada do piloto:
...
- aba `Concluidas` validada com agrupamento mensal e abertura de itens no dialog
- filtros:
  - `Todos os workflows`
  - `Somente workflow ativo`
  validados nas listas da rota
- `Minhas solicitacoes` validada com agrupamento mensal coerente
- persistencia dos anexos validada no Firestore

### Resultado da etapa

A Etapa 7 ficou concluida com:

- workflow 3 operando ponta a ponta na mesma UI
- aba `Concluidas` consumindo o read-side ja existente
...
- sem reintroduzir dependencia do frontend legado

### Fechamento da Fase 1

Com a validacao manual completa da Etapa 7, a Fase 1 pode ser considerada concluida para o recorte de Facilities e Suprimentos.
