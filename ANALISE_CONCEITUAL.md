...
-   **Minhas Tarefas (`/me/tasks`):** Uma caixa de entrada pessoal para o colaborador, onde são listadas todas as solicitações de workflow que foram atribuídas a ele para processamento.
...
-   **Gerenciamento de Workflows (`/admin/workflows`):** O centro de controle para todos os processos digitais da empresa.
    -   **Aba "Definições":** Permite gerenciar as "Áreas de Workflow" (os botões guarda-chuva) e as "Definições de Workflow" (onde se constrói o formulário, as etapas, as regras de SLA e as notificações de cada processo).
    -   **Aba "Visão Geral":** Uma tabela completa com todas as solicitações já feitas no sistema, permitindo uma visão global e auditoria de todos os processos.
