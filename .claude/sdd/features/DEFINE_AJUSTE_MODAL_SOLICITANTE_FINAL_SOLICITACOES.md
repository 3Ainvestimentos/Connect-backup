...
| M4 | O bloco final deve se chamar `Histórico`, usar apresentação inspirada na linha do tempo do legado e consumir a fonte de dados correta por origem | O bloco final mostra eventos com o item mais antigo em cima e o item mais recente embaixo, com marcadores/etapas e metadados legíveis. Para chamados `v2`, a fonte primária do histórico são as etapas estruturadas de `progress` (sem detalhamento de timeline). Para chamados `legacy`, a fonte é a `timeline` derivada pelo adapter. O usuário vê apenas etapas; não há detalhamento evento-a-evento da timeline na experiência final de chamados `v2` |
...
| # | Requirement | Acceptance Criteria |
|---|-------------|-------------------|
| S1 | A lógica que transforma `progress` (v2) ou `timeline` (legacy) em `Histórico` deve ficar isolada do JSX principal | O componente de dialog consome um adapter/helper de apresentação que bifurca por `detail.origin`: quando `v2`, extrai etapas de `progress`; quando `legacy`, extrai eventos de `timeline`. A derivação não ocorre inline no render |
...
| 1.1 | 2026-04-17 | iterate-agent | Corrige M4 e S1: explicita bifurcacao de fonte por origem — v2 usa `progress` (etapas estruturadas) como fonte primaria, legacy usa `timeline` derivada. Alinha com distincao ja capturada no BRAINSTORM (Must Have) |
